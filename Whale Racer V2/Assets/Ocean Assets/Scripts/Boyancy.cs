...
public class Boyancy : MonoBehaviour {
	...
	public Renderer _renderer ; //Makes objects appear on the screen ->
    ...
    void Start () {
        ...
        //hiw = rrigidbody.GetComponent<HeightInWater>();
    ...
    private void ResetBuoyancy()
    {
        ...
        for (int x = 0; x < SlicesX; x++)
        {
            for (int y = 0; y < SlicesX; y++)
            {
                ...
                if (!underWater)
                {
                    //float tempHeight = dynamicHeight;

                    //while (Mathf.Abs(tempHeight) != 0.0f)
                    //{
                       // tempHeight -= 0.2f;
                       // blobs.Add(1.0f, tempHeight, 1.0f); //below sea level --> y-componet = 0.0f usually
                    //}
                    blobs.Add(new Vector3((-0.5f + x * xstep) * width, -0.5f, (-0.5f + y * ystep) * length) + Vector3.up * ypos); //below sea level --> y-componet = 0.0f usually
                ...
                else
                {
                    ...
                    //while(Mathf.Abs(tempHeight) != 20.0f)
                    //{
                    // tempHeight += 0.2f;
                    blobs.Add(new Vector3((-0.5f + x * xstep) * width, 14.0f, (-0.5f + y * ystep) * length) + Vector3.up * ypos); //below sea level --> y-componet = 0.0f usually
                   // }
                    //blobs.Add(new Vector3((-0.5f + x * xstep) * width, 20.0f, (-0.5f + y * ystep) * length) + Vector3.up * ypos); //below sea level --> y-componet = 0.0f usually
                    prevUnderWater = underWater;
                }
