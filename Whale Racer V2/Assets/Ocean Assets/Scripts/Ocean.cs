...
public class Ocean : MonoBehaviour {
    ...
    //projection members
    /*public float fps = 30.0f;  //frames per second
    public Texture2D caustic;  //these are the caustic textures
    public int frameIndex;
    private Projector projector;
    private new Renderer renderer;*/ //not sure whether to use renderer or projector -> going with renderer for caustic illumination
    ...
    {
        Debug.Log("Whale Fin or Fluke Hit!");
    }

    private void OnTriggerExit (Collider other)
    {
        Debug.Log("Make Some Splashes With Fluke or Fin Exit!");
    }
	...
	void Initialize(bool runtimeLoad = false) {
        ...
        ExactWaterHeight.wHeight = GetWaterHeightAtLocation(player.position.x, player.position.z);		

		//force the main camera to draw depth if we want shore lines to show (Not needed for deferred rendering paths.)
		if(forceDepth) Camera.main.depthTextureMode = DepthTextureMode.Depth; //Commented out
	}
