...
public class OceanGeneratorInspector : Editor {
    ...
    //public static Texture2D oldcaustic;  //these are the caustic textures
	...
	public override void OnInspectorGUI() {
		...
		if(editormode==0) {
			...
			//currentPreset = EditorGUILayout.Popup(currentPreset, presets); //Commented out removes panel error
			...
			if(ocean.hasFog1 || ocean.hasFog2) {
                ...
                /*EditorGUILayout.BeginHorizontal();
                EditorGUILayout.LabelField("Caustic:");
                GUILayout.Space(-120);
                //GUI.contentColor = new Color(1f, 1f, 1f, 1f);
                //Texture2D myTexture = AssetPreview.GetAssetPreview(object);
                //ocean.caustic = AssetPreview.GetAssetPreview(ocean.caustic);
                ocean.caustic = (Texture2D)EditorGUILayout.ObjectField(ocean.caustic, typeof(Texture2D), true, GUILayout.MinWidth(120));
                if (GUILayout.Button("?", GUILayout.MaxWidth(20)))
                {
                    EditorUtility.DisplayDialog("Caustics", "The caustics are the animated light sources on the ocean floor, like shadows projected.\n\n" +
                    "Use this to project textures", "OK");
                }
                EditorGUILayout.EndHorizontal();*/
            }
        ...
        //if(ocean.caustic != oldcaustic) { if (ocean.caustic != null) ocean.caustic.Set("_EmissionMap", ocean.caustic); oldcaustic = ocean.caustic; }
        //ocean.caustic = oldcaustic;
        //oldcaustic = ocean.caustic;
...
public void DrawHalfSeparator(bool left = true) {
	...
	public void DrawBackground() {
		if (Event.current.type == EventType.Repaint) {
            ...
            //GUI.DrawTexture(new Rect(0f, rect.yMin + 6f, Screen.width, 950), tex); //Commented out removes panel error
            GUI.color = Color.white;
		}
	}
